...
class ConfigurationWindow:
    """FIXED Configuration management window with proper close handling"""
    ...
    def on_close(self):
        ...
        if self.on_save_callback:
            # Call the parent's cleanup callback
            self.on_save_callback(None, close_only=True)
        self.window.destroy()
    ...
    def create_widgets(self):
        ...
        notebook.add(app_frame, text="Application Settings")
        ...
        ttk.Button(button_frame, text="Cancel", command=self.on_close).pack(side='right', padx=(5, 0))
    ...
    def create_source_tab(self, parent):
        ...
        ttk.Entry(fm_frame, textvariable=self.fm_dsn_var, width=40).grid(row=0, column=1, padx=(10, 0), pady=2, sticky='ew')
        
        ttk.Label(fm_frame, text="Username:").grid(row=1, column=0, sticky='w', pady=2)
        self.fm_user_var = tk.StringVar()
        ttk.Entry(fm_frame, textvariable=self.fm_user_var, width=40).grid(row=1, column=1, padx=(10, 0), pady=2, sticky='ew')
        
        ttk.Label(fm_frame, text="Password:").grid(row=2, column=0, sticky='w', pady=2)
        self.fm_pwd_var = tk.StringVar()
        ttk.Entry(fm_frame, textvariable=self.fm_pwd_var, width=40, show='*').grid(row=2, column=1, padx=(10, 0), pady=2, sticky='ew')
    ...
    def create_target_tab(self, parent):
        ...
        ttk.Entry(sb_frame, textvariable=self.sb_host_var, width=50).grid(row=0, column=1, padx=(10, 0), pady=2, sticky='ew')
        
        ttk.Label(sb_frame, text="Database:").grid(row=1, column=0, sticky='w', pady=2)
        self.sb_db_var = tk.StringVar()
        ttk.Entry(sb_frame, textvariable=self.sb_db_var, width=50).grid(row=1, column=1, padx=(10, 0), pady=2, sticky='ew')
        
        ttk.Label(sb_frame, text="Username:").grid(row=2, column=0, sticky='w', pady=2)
        self.sb_user_var = tk.StringVar()
        ttk.Entry(sb_frame, textvariable=self.sb_user_var, width=50).grid(row=2, column=1, padx=(10, 0), pady=2, sticky='ew')
        
        ttk.Label(sb_frame, text="Password:").grid(row=3, column=0, sticky='w', pady=2)
        self.sb_pwd_var = tk.StringVar()
        ttk.Entry(sb_frame, textvariable=self.sb_pwd_var, width=50, show='*').grid(row=3, column=1, padx=(10, 0), pady=2, sticky='ew')
        
        ttk.Label(sb_frame, text="Port:").grid(row=4, column=0, sticky='w', pady=2)
        self.sb_port_var = tk.StringVar()
        ttk.Entry(sb_frame, textvariable=self.sb_port_var, width=50).grid(row=4, column=1, padx=(10, 0), pady=2, sticky='ew')
    ...
    def create_export_tab(self, parent):
        ...
        ttk.Entry(path_frame, textvariable=self.export_path_var, width=40).pack(side='left', fill='x', expand=True)
    ...
    def create_app_tab(self, parent):
        """NEW: Create application settings tab"""
        # Application settings
        app_frame = ttk.LabelFrame(parent, text="Application Settings", padding=10)
        app_frame.pack(fill='x', padx=10, pady=10)
        
        # Refresh interval
        ttk.Label(app_frame, text="Migration Status Refresh (seconds):").grid(row=0, column=0, sticky='w', pady=2)
        self.refresh_interval_var = tk.StringVar()
        ttk.Entry(app_frame, textvariable=self.refresh_interval_var, width=10).grid(row=0, column=1, padx=(10, 0), pady=2, sticky='w')
        
        # Debug mode
        self.debug_var = tk.BooleanVar()
        ttk.Checkbutton(app_frame, text="Enable Debug Mode", variable=self.debug_var).grid(row=1, column=0, columnspan=2, sticky='w', pady=5)
        
        # Auto-scroll logs
        self.autoscroll_var = tk.BooleanVar()
        ttk.Checkbutton(app_frame, text="Auto-scroll Log Viewer", variable=self.autoscroll_var).grid(row=2, column=0, columnspan=2, sticky='w', pady=5)
    ...
    def load_config_values(self):
        ...
        try:
            ...
            if config_file.exists():
                ...
                # Source database
                source_config = toml_config.get('database', {}).get('source', {})
                self.fm_dsn_var.set(source_config.get('dsn', ''))
                self.fm_user_var.set(source_config.get('user', ''))
                self.fm_pwd_var.set(source_config.get('pwd', ''))
                
                # Target database
                target_config = toml_config.get('database', {}).get('target', {})
                supabase_config = target_config.get('supabase', {})
                self.sb_host_var.set(target_config.get('host', ''))
                self.sb_db_var.set(target_config.get('dsn', ''))
                self.sb_user_var.set(supabase_config.get('user', ''))
                self.sb_pwd_var.set(supabase_config.get('pwd', ''))
                self.sb_port_var.set(supabase_config.get('port', '5432'))
                
                # Export settings
                export_config = toml_config.get('export', {})
                self.export_path_var.set(export_config.get('path', ''))
                self.export_prefix_var.set(export_config.get('prefix', 'rat'))
                
                # Image formats
                formats = export_config.get('image_formats_supported', ['jpg'])
                self.jpg_var.set('jpg' in formats)
                self.webp_var.set('webp' in formats)
                
        except Exception as e:
            print(f"Could not load TOML config: {e}")
        
        # Application settings with defaults
        self.refresh_interval_var.set(self.config.get('refresh_interval', '10'))
        self.debug_var.set(self.config.get('debug_mode', False))
        self.autoscroll_var.set(self.config.get('autoscroll_logs', True))
    ...
    def save_config(self):
        ...
        self.config.update({
            ...
            'autoscroll_logs': self.autoscroll_var.get(),
            'image_formats': []
        })
    ...
    def test_connections(self):
        ...
        messagebox.showinfo("Test Connections", 
                          "Connection testing will be implemented using the main application's test functions.")
...
class FileMakerSyncGUI:
    ...
    def __init__(self, root):
        ...
        # Windows - FIXED: Better window management
        self.child_windows = {}
    ...
    def create_widgets(self):
        """Create the main dashboard layout - FIXED LAYOUT"""
        # Configure root
        ...
        # Connection status row - SIMPLIFIED
        self.create_connection_status(main_container)
        
        # FIXED: Reordered - Quick Actions above Migration Overview
        # Main content area
        content_frame = ttk.Frame(main_container)
        content_frame.pack(fill='both', expand=True)
        
        # Left side - Quick Actions and Migration Overview
        ...
        # Migration Overview below
        overview_frame = ttk.LabelFrame(left_frame, text=" Migration Overview ", padding=10)
        overview_frame.pack(fill='both', expand=True)
        
        self.migration_overview = MigrationOverview(overview_frame)
        ...
        # Activity controls
        activity_header = ttk.Frame(right_frame)
        activity_header.pack(fill='x', pady=(0, 5))
        
        ttk.Label(activity_header, text="Activity", font=('Arial', 12, 'bold')).pack(side='left')
        
        # Clock icon button for detailed activity view
        self.activity_detail_btn = ttk.Button(activity_header, text="🕒", width=3, 
                                            command=self.open_activity_detail)
        self.activity_detail_btn.pack(side='right')
        
        # Recent Activity (simplified)
        self.recent_activity = RecentActivity(right_frame, height=15)
    ...
    def create_header(self, parent):
        ...
        # FIXED: Add Cancel button in header
        control_frame = ttk.Frame(header_frame)
        control_frame.pack(side='right')
        
        self.cancel_button = ttk.Button(control_frame, text="Cancel Operation", 
                                      command=self.cancel_operation, state='disabled')
        self.cancel_button.pack(side='right', padx=(10, 0))
    
    def create_connection_status(self, parent):
        """SIMPLIFIED connection status section"""
        conn_frame = ttk.Frame(parent)
        conn_frame.pack(fill='x', pady=(0, 20))
        
        # FileMaker status - simplified
        fm_frame = ttk.LabelFrame(conn_frame, text=" FileMaker Pro ", padding=10)
        fm_frame.pack(side='left', fill='x', expand=True, padx=(0, 10))
        
        self.fm_status_text = tk.StringVar(value="Not tested")
        fm_label = ttk.Label(fm_frame, textvariable=self.fm_status_text)
        fm_label.pack(side='left')
        
        ttk.Button(fm_frame, text="Details", width=8, 
                  command=self.show_fm_details).pack(side='right')
        ttk.Button(fm_frame, text="Test", width=8, 
                  command=self.test_filemaker_connection).pack(side='right', padx=(0, 5))
        
        # Target status - simplified  
        target_frame = ttk.LabelFrame(conn_frame, text=" Supabase Target ", padding=10)
        target_frame.pack(side='right', fill='x', expand=True, padx=(10, 0))
        
        self.target_status_text = tk.StringVar(value="Not tested")
        target_label = ttk.Label(target_frame, textvariable=self.target_status_text)
        target_label.pack(side='left')
        
        ttk.Button(target_frame, text="Details", width=8, 
                  command=self.show_target_details).pack(side='right')
        ttk.Button(target_frame, text="Test", width=8, 
                  command=self.test_target_connection).pack(side='right', padx=(0, 5))
    ...
    def on_new_log_entry(self, log_entry):
        """Handle new log entries - IMPROVED"""
        # Only add GUI-relevant activities to the activity log
        if log_entry.component in ['Application', 'Operation', 'Connection']:
            activity_msg = f"{log_entry.component}: {log_entry.message}"
            self.recent_activity.add_activity(activity_msg)
    ...
    def on_operation_status(self, status, operation, result=None):
        """Handle operation status updates - IMPROVED"""
        if status == 'start':
            ...
            result_text = "Successfully" if result == 'success' else "Failed"
            self.recent_activity.add_activity(f"Completed: {operation.replace('_', ' ')} - {result_text}")
            
            # Refresh status after operation
            self.root.after(2000, self.refresh_migration_status)
        ...
        if self.config_file.exists():
            try:
                with open(self.config_file, 'r') as f:
                    ...
                    config.setdefault('refresh_interval', '10')
                    config.setdefault('debug_mode', False)
                    config.setdefault('autoscroll_logs', True)
                    return config
            except Exception as e:
    ...
    # Connection testing methods - IMPROVED
    ...
    def test_target_connection(self):
        ...
        self.recent_activity.add_activity("Testing target connection...")
    ...
    def on_connection_test_complete(self, connection_type, status):
        """Handle connection test completion - SIMPLIFIED"""
        def update_ui():
            if connection_type == 'filemaker':
                status_text = "✓ Connected" if status['connected'] else "✗ Failed"
                self.fm_status_text.set(status_text)
            elif connection_type == 'target':
                status_text = "✓ Connected" if status['connected'] else "✗ Failed"
                self.target_status_text.set(status_text)
            ...
            result_text = "Success" if status['connected'] else "Failed"
            self.recent_activity.add_activity(f"{connection_type.title()} test: {result_text}")
            
            self.update_status_indicator()
        
        self.root.after(0, update_ui)
    
    def show_fm_details(self):
        """Show FileMaker connection details"""
        status = self.connection_tester.connection_status['filemaker']
        detail_text = f"FileMaker Pro Connection Details\n\n"
        detail_text += f"Status: {'Connected' if status['connected'] else 'Disconnected'}\n"
        detail_text += f"Message: {status['message']}\n"
        
        messagebox.showinfo("FileMaker Connection Details", detail_text)
    
    def show_target_details(self):
        """Show target connection details"""
        status = self.connection_tester.connection_status['target']
        detail_text = f"Supabase Target Connection Details\n\n"
        detail_text += f"Status: {'Connected' if status['connected'] else 'Disconnected'}\n"
        detail_text += f"Message: {status['message']}\n"
        
        messagebox.showinfo("Supabase Connection Details", detail_text)
    
    # Operation methods - IMPROVED
    def run_operation(self, operation: str):
        """Run a migration operation with warnings"""
        ...
        # FIXED: Warning for full sync
        if operation == 'full_sync':
            warning_msg = ("WARNING: Full Sync Operation\n\n"
                          "This operation will:\n"
                          "• Remove ALL existing data from the target database\n"
                          "• Take a long time to complete (possibly hours)\n"
                          "• Cannot be easily undone\n\n"
                          "Have you considered using 'Incremental Sync' instead?\n\n"
                          "Do you want to continue with Full Sync?")
            
            if not messagebox.askyesno("Full Sync Warning", warning_msg, icon='warning'):
                return
        
        # Standard confirmation for other operations
        elif not messagebox.askyesno("Confirm Operation", 
                                   f"Are you sure you want to run {operation.replace('_', ' ')}?"):
            return
        
        # Add debug flag if enabled
        debug_args = ['--debug'] if self.config.get('debug_mode', False) else []
        
        self.operation_manager.run_operation_async(operation, debug_args)
    
    def cancel_operation(self):
        """Cancel current operation"""
        if self.current_operation:
            if messagebox.askyesno("Cancel Operation", 
                                 f"Are you sure you want to cancel '{self.current_operation.replace('_', ' ')}'?"):
                # This would require implementing cancellation in the operation manager
                self.recent_activity.add_activity(f"Cancelled: {self.current_operation.replace('_', ' ')}")
                self.current_operation = None
                self.cancel_button.configure(state='disabled')
                self.quick_actions.hide_progress()
            ...
            def update_ui():
                if success:
                    ...
                    if 'filemaker' in conn_status:
                        ...
                        fm_connected = conn_status['filemaker']['connected']
                        self.fm_status_text.set("✓ Connected" if fm_connected else "✗ Failed")
                    
                    if 'target' in conn_status:
                        self.connection_tester.connection_status['target'] = conn_status['target']
                        target_connected = conn_status['target']['connected']
                        self.target_status_text.set("✓ Connected" if target_connected else "✗ Failed")
                    
                    self.update_connection_displays()
                else:
                    self.recent_activity.add_activity(f"Status refresh failed: {data}")
    ...
    def open_window(self, window_type, window_class, *args, **kwargs):
        """Generic window opener with proper cleanup"""
        if window_type in self.child_windows and self.child_windows[window_type]:
            try:
                # Try to raise existing window
                self.child_windows[window_type].window.lift()
                return
            except tk.TclError:
                # Window was destroyed, remove reference
        ...
        window.window.protocol("WM_DELETE_WINDOW", lambda: [cleanup_callback(), window.window.destroy()])
        self.child_windows[window_type] = window
    
    # Menu actions - FIXED
    def open_configuration(self):
        """Open configuration window"""
        def on_config_saved(new_config, close_only=False):
            if not close_only and new_config:
            ...
            self.child_windows['config'] = None
        
        if 'config' in self.child_windows and self.child_windows['config']:
            try:
                self.child_windows['config'].window.lift()
                return
            except tk.TclError:
                self.child_windows['config'] = None
        
        self.child_windows['config'] = ConfigurationWindow(self.root, self.config, on_config_saved)
    
    def open_log_viewer(self):
        """Open log viewer window"""
        self.open_window('log_viewer', LogViewerWindow, self.log_manager)
    
    def open_log_stats(self):
        """Open log statistics window"""
        self.open_window('log_stats', LogStatsWindow, self.log_manager)
    
    def open_activity_detail(self):
        """Open detailed activity window"""
        # This could be a specialized log viewer showing only GUI activities
    ...
    def show_diagnostic_results(self, results):
        ...
        diag_window.protocol("WM_DELETE_WINDOW", diag_window.destroy)
...
Terminology:
• Full Sync: Complete migration with DDL+DML (destructive)
• Incremental Sync: Data-only migration (DML only)
...
"""
    ...
    def start_auto_refresh(self):
        ...
        try:
            interval_seconds = int(self.config.get('refresh_interval', '10'))
            interval_ms = max(5, interval_seconds) * 1000  # Minimum 5 seconds
        ...
        def auto_refresh():
            ...
            self._refresh_timer = self.root.after(interval_ms, auto_refresh)
